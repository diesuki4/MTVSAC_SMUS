...
/// Define Joint points
/// </summary>
public class PhotonVNectBarracudaRunner : MonoBehaviourPunCallbacks, IPunObservable
{
    /// <summary>
    ...
    private IEnumerator WaitLoad()
    {
        ...
        // Init VNect model
        photonView.RPC("InitPose", RpcTarget.All, offset3D, heatMap3D);
    ...
    */
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.IsWriting)
        {
            stream.SendNext(offset3D);
            stream.SendNext(heatMap3D);
        }
        else
        {
            offset3D = (float[])stream.ReceiveNext();
            heatMap3D = (float[])stream.ReceiveNext();
        }
    }
}
